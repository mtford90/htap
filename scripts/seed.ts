#!/usr/bin/env npx tsx
/**
 * Seed script — populates the local .httap database with realistic example requests.
...
const nodeRequests: SeedRequest[] = [
  {
    ...
    source: "node",
    method: "GET",
    url: "https://api.github.com/repos/mtford90/httap",
    host: "api.github.com",
    path: "/repos/mtford90/httap",
  ...
  },
  {
    ...
    source: "node",
    method: "GET",
    url: "https://registry.npmjs.org/httap",
...
const curlRequests: SeedRequest[] = [
  ...
  },
  {
    ...
    responseStatus: 200,
    responseHeaders: { "content-type": "application/json" },
    responseBody: JSON.stringify({ headers: { "X-Custom-Header": "hello-httap" } }),
    durationMs: 110,
  },
  {
    sessionId: "seed-curl",
...
const savedRequests: SeedRequest[] = [
  ...
  },
  {
    ...
    source: "python3",
    method: "GET",
    url: "https://api.github.com/repos/mtford90/httap/issues?state=open",
    host: "api.github.com",
    path: "/repos/mtford90/httap/issues?state=open",
    requestHeaders: jsonHeaders({
...
function seed(): void {
  console.log("httap seed script");
  console.log("==================\n");

  // Ensure .httap directory exists
  ensureHttapDir(PROJECT_ROOT);
  const { databaseFile } = getHttapPaths(PROJECT_ROOT);
  ...
  console.log("\nRun 'httap tui' to inspect the seeded data.");
