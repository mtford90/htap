...
3. **Start intercepting**: `eval "$(htpx vars)"` in the project directory (or `htpx on` if the shell wrapper is installed)
...
Always call `htpx_get_status` first to confirm the daemon is running. If it is not running, try to start it by running `eval "$(htpx vars)"` in the project directory. If htpx is not installed, follow the Installation & Setup steps above.
