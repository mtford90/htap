# httap Code Review -- 2026-02-09

Review of the Phase 2 "Config-as-code Interceptors" feature: jiti-based TypeScript loader, deferred forward() pattern, runner lifecycle, HttapClient, CLI commands, MCP tools, TUI indicators, and storage extensions.
...
**Positive Observations:**
- The interceptor runner tests are thorough and well-structured: mock, modify, observe, pass-through, error handling, forward() idempotency, cleanup/abort, request immutability, context.log, and context.httap are all covered.
...
**Positive Observations:**
- The `src/interceptors.ts` barrel file cleanly re-exports only the consumer-facing types (`Interceptor`, `InterceptorRequest`, `InterceptorResponse`, `InterceptorContext`, `HttapClient`), avoiding leaking internal types.
...
- [ ] **INT.5.1: `httap-client.ts` is in `daemon/` but implements a `shared/types.ts` interface**

  **Severity:** Low

  **File:** `src/daemon/httap-client.ts`

  **Issue:** `createHttapClient` lives in `daemon/` because it depends on `RequestRepository` (a daemon module). This is correct -- it cannot be in `shared/`. However, the `HttapClient` interface is exported from `shared/types.ts` and re-exported via `src/interceptors.ts` for external consumers. If a future consumer wants to create an `HttapClient` outside the daemon (e.g. in an MCP context or test harness), they have no factory function available from the public API. Currently this is fine because only the daemon creates `HttapClient` instances, but it is worth noting as a potential friction point.
...
- [ ] **INT.6.1: Arbitrary TypeScript code execution via interceptor files**
  ...
  **Fix:** This is a known trade-off of config-as-code. Consider:
  ...
  2. Adding `.httap/interceptors/` to the default `.gitignore` template (if not already) to reduce the risk of committing untrusted interceptor code.
...
- [ ] **INT.6.2: MCP `httap_reload_interceptors` tool allows remote code reload**
  ...
  **Issue:** An MCP client (e.g. an AI agent) can trigger an interceptor reload via this tool. If the agent has also modified files in `.httap/interceptors/` (either directly or via another tool), this effectively allows remote code injection into the daemon. The MCP tools were originally read-only, but `httap_reload_interceptors` and `httap_clear_requests` are mutating operations.
...
- [ ] **INT.6.3: Interceptor handler has unrestricted access to `HttapClient`**

  **Severity:** Medium

  **File:** `src/daemon/httap-client.ts:1-30`

  **Issue:** The `HttapClient` passed to interceptor handlers provides read access to all captured traffic, including request/response bodies, headers (which may contain auth tokens), and metadata. A malicious interceptor could exfiltrate all captured traffic. This is inherent to the design (interceptors need to query traffic to make decisions), but there is no mechanism to scope access -- every interceptor sees everything.
...
- The `httap interceptors init` command scaffolds a well-commented example file covering all three patterns (mock, modify, observe).
...
- [x] **INT.7.2: No feedback when `interceptors reload` is called but interceptors directory does not exist**
  ...
  **Fix:** Return an error message in the response (e.g. `{ success: false, count: 0, error: "Interceptors directory not found. Create .httap/interceptors/ and restart the daemon." }`), and display it in the CLI.
...
- [x] **INT.8.1: `findMatchingInterceptor` evaluates match functions sequentially**
  ...
  **Issue:** Match functions are evaluated sequentially with `await`. If a user has many interceptors with async match functions (e.g. querying the database via `ctx.httap`), the total matching time is the sum of all match evaluations, not the max. With the 5-second `MATCH_TIMEOUT_MS`, 10 interceptors with slow match functions could add up to 50 seconds of latency per request. This is unlikely in practice (match functions should be fast predicates), but the design allows it.
