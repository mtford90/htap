...
- [x] **`htap_count_requests`** — expose existing `ControlClient.countRequests()`. Accepts same filter params as `htap_list_requests`.
- [x] **`htap_clear_requests`** — expose existing `ControlClient.clearRequests()`. No params.
- [x] **`htap_list_sessions`** — add `listSessions()` to `ControlClient` (handler already exists on control server).
- [x] **Batch `htap_get_request`** — accept comma-separated IDs, return multiple formatted requests.
...
- [ ] **JSON body querying** — new tool `htap_query_json` with `json_path` (e.g. `$.data.users`), optional `value`, `target` (`"request"` | `"response"` | `"both"`). Uses SQLite `json_extract()`.
