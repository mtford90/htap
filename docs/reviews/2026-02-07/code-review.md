...
- [ ] **F3.3: project init command bypasses resolveOverridePath**
  ...
  **Issue:** The `project init` command handles `--dir` with a simple `path.resolve()`, which does not expand `~` to the home directory. Every other command uses `findProjectRoot` or `findOrCreateProjectRoot`, which call `resolveOverridePath` internally. This means `httap -d ~/my-project project init` will create `.httap` in a literal `~/my-project` directory relative to cwd rather than in the user's home directory.

  **Fix:** Use `resolveOverridePath` or create a `resolveDir` helper exported from project.ts. Alternatively, reuse `findOrCreateProjectRoot(undefined, globalOpts.dir)` and then call `ensureHttapDir`.

  **Quick win:** Yes -- replace with `findOrCreateProjectRoot` + `ensureHttapDir`.
...
- [ ] **F3.7: --dir option not validated for path traversal**
  ...
  **Issue:** The `--dir` option accepts any path, including symlinks or paths that resolve outside the user's control (e.g. `/etc`). Since httap creates directories (`.httap/`) and writes files (database, CA certificates) in the resolved path, an attacker with control over the CLI arguments could cause writes to unintended locations.
...
- [ ] **F3.8: No feedback when --dir points to non-existent directory**
  ...
  **Issue:** When `--dir /nonexistent/path` is passed, `findProjectRoot` returns `undefined` because neither `.httap` nor `.git` exists at that path. The error message says "Not in a project directory" which doesn't mention the `--dir` flag, leaving the user confused about why their override didn't work.

  **Fix:** Check whether the override path exists first and provide a specific error message: `"Directory not found: /nonexistent/path"` or `"No .httap or .git found at /nonexistent/path (specified via --dir)"`.
