/**
 * `httap completions <shell>` — generate shell completion scripts.
 *
...
function generateZshCompletions(program: Command): string {
  ...
  return `#compdef httap

# Auto-generated by httap completions zsh
# Add to your .zshrc: eval "$(httap completions zsh)"
...
_httap() {
  ...
  case "$state" in
    command)
      _describe -t commands 'httap command' commands
      ;;
    args)
      case "$words[1]" in
${subcmdCases}
...
function generateBashCompletions(program: Command): string {
  ...
  return `# Auto-generated by httap completions bash
# Add to your .bashrc: eval "$(httap completions bash)"
...
function generateFishCompletions(program: Command): string {
  const commands = collectCommands(program);
  const lines: string[] = [
    "# Auto-generated by httap completions fish",
    "# Add to your config: httap completions fish | source",
    "",
  ...
  for (const cmd of commands) {
    lines.push(
      `complete -c httap -n '__fish_use_subcommand' -a '${escapeForShell(cmd.name)}' -d '${escapeForShell(cmd.description)}'`
    );
  }
  ...
  for (const cmd of commands) {
    for (const opt of cmd.options) {
      ...
      lines.push(
        `complete -c httap -n '__fish_seen_subcommand_from ${cmd.name}' -l '${escapeForShell(flag)}' -d '${escapeForShell(opt.description)}'`
      );
    }
    for (const sub of cmd.subcommands) {
      lines.push(
        `complete -c httap -n '__fish_seen_subcommand_from ${cmd.name}' -a '${escapeForShell(sub.name)}' -d '${escapeForShell(sub.description)}'`
      );
    }
  }
