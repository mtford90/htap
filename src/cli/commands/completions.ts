/**
 * `htap completions <shell>` — generate shell completion scripts.
 *
...
# Auto-generated by htap completions zsh
# Add to your .zshrc: eval "$(htap completions zsh)"
...
_htap() {
  ...
  case "$state" in
    command)
      _describe -t commands 'htap command' commands
      ;;
    args)
      case "$words[1]" in
${subcmdCases}
...
function generateBashCompletions(program: Command): string {
  ...
  return `# Auto-generated by htap completions bash
# Add to your .bashrc: eval "$(htap completions bash)"
...
function generateFishCompletions(program: Command): string {
  const commands = collectCommands(program);
  const lines: string[] = [
    "# Auto-generated by htap completions fish",
    "# Add to your config: htap completions fish | source",
    "",
  ...
  for (const cmd of commands) {
    lines.push(
      `complete -c htap -n '__fish_use_subcommand' -a '${escapeForShell(cmd.name)}' -d '${escapeForShell(cmd.description)}'`
    );
  }
  ...
  for (const cmd of commands) {
    for (const opt of cmd.options) {
      ...
      lines.push(
        `complete -c htap -n '__fish_seen_subcommand_from ${cmd.name}' -l '${escapeForShell(flag)}' -d '${escapeForShell(opt.description)}'`
      );
    }
    for (const sub of cmd.subcommands) {
      lines.push(
        `complete -c htap -n '__fish_seen_subcommand_from ${cmd.name}' -a '${escapeForShell(sub.name)}' -d '${escapeForShell(sub.description)}'`
      );
    }
  }
