/**
 * `httap requests` — list, search, query, count, and clear captured requests.
 */
...
searchSubcommand.action(
  async (
    ...
    try {
      ...
      const hint = formatHint(["httap request <id> for detail", "--json for JSON output"]);
  ...
  async (opts: RequestsFlags & { limit?: string; offset?: string }, command: Command) => {
    ...
    try {
      ...
      if (summaries.length === 0) {
        ...
        const hint = formatHint(["make HTTP requests while httap is intercepting"]);
        if (hint) console.log(hint);
        return;
      }
