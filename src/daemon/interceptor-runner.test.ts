...
import type { HttapClient, InterceptorRequest, InterceptorResponse } from "../shared/types.js";
...
function createMockHttapClient(): HttapClient {
  return {
...
describe("interceptor-runner", () => {
  let mockHttapClient: HttapClient;
  let projectRoot: string;

  beforeEach(() => {
    mockHttapClient = createMockHttapClient();
  ...
  describe("createInterceptorRunner", () => {
    ...
    describe("context.httap", () => {
      it("should provide httap client to handler", async () => {
        const interceptor: LoadedInterceptor = {
          name: "httap-test",
          handler: async (ctx) => {
        ...
        expect(mockHttapClient.countRequests).toHaveBeenCalled();
      });
    });
