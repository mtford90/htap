...
describe("loadConfig", () => {
  ...
  beforeEach(() => {
    ...
    fs.mkdirSync(path.join(tempDir, ".httap"), { recursive: true });
  });
  ...
  it("loads and merges partial config", () => {
    fs.writeFileSync(
      path.join(tempDir, ".httap", "config.json"),
  ...
  it("loads full config", () => {
    ...
    fs.writeFileSync(path.join(tempDir, ".httap", "config.json"), JSON.stringify(custom));
  ...
  it("returns defaults on malformed JSON", () => {
    fs.writeFileSync(path.join(tempDir, ".httap", "config.json"), "not valid json {{{");
  ...
  it("returns defaults when config is an array", () => {
    fs.writeFileSync(path.join(tempDir, ".httap", "config.json"), JSON.stringify([1, 2, 3]));
  ...
  it("returns defaults when config is null", () => {
    fs.writeFileSync(path.join(tempDir, ".httap", "config.json"), "null");
