...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "procsi-browser-test-"));
});
...
describe("generateFirefoxExtension", () => {
  ...
  it("background.js contains session ID and token", () => {
    ...
    expect(bg).toContain("x-procsi-internal-session-id");
    expect(bg).toContain("sess-99");
    expect(bg).toContain("x-procsi-internal-session-token");
  ...
  it("manifest includes gecko extension ID", () => {
    ...
    expect(geckoSettings?.["id"]).toBe("procsi@procsi.dev");
  });
});
