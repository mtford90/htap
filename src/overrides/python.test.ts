...
describe("generatePythonOverrideScript", () => {
  ...
  it("includes httap comment header", () => {
    const script = generatePythonOverrideScript("/path/to/ca.pem");
    expect(script).toContain("# Generated by httap");
  });

  it("embeds the CA cert path correctly", () => {
    const caCertPath = "/Users/test/.httap/ca.pem";
  ...
  it("escapes paths with backslashes (Windows)", () => {
    const caCertPath = "C:\\Users\\test\\.httap\\ca.pem";
    const script = generatePythonOverrideScript(caCertPath);
    // Backslashes should be doubled
    expect(script).toContain(`httplib2.CA_CERTS = 'C:\\\\Users\\\\test\\\\.httap\\\\ca.pem'`);
  });

  it("handles paths with both backslashes and quotes", () => {
    const caCertPath = "C:\\Program Files\\httap's data\\ca.pem";
    const script = generatePythonOverrideScript(caCertPath);
    expect(script).toContain(
      `httplib2.CA_CERTS = 'C:\\\\Program Files\\\\httap\\'s data\\\\ca.pem'`
    );
  });
});
...
describe("writePythonOverride", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "httap-python-test-"));
  });
  ...
  it("writes correct content to file", () => {
    ...
    expect(content).toContain("# Generated by httap");
