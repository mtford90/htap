...
describe("generateRubyOverrideScript", () => {
  it("contains expected OpenSSL monkey-patch", () => {
    ...
    expect(script).toContain("add_file(htap_ca_path)");
  });
  ...
  it("includes descriptive comment", () => {
    ...
    expect(script).toContain("# Generated by htap — ensures Ruby trusts the proxy CA certificate");
  });

  it("embeds the CA cert path correctly", () => {
    const caCertPath = "/home/user/.htap/ca.pem";
    const script = generateRubyOverrideScript(caCertPath);

    expect(script).toContain(`htap_ca_path = '${caCertPath}'`);
    expect(script).toContain("File.exist?(htap_ca_path)");
  });
  ...
  it("handles paths with special characters", () => {
    ...
    expect(script).toContain("htap_ca_path = '/path/with spaces/and\\'quotes/ca.pem'");
  });
  ...
  it("handles paths with backslashes", () => {
    ...
    expect(script).toContain("htap_ca_path = 'C:\\Users\\test\\ca.pem'");
  });

  it("includes file existence check", () => {
    const script = generateRubyOverrideScript("/path/to/ca.pem");

    expect(script).toContain("if File.exist?(htap_ca_path)");
  });
});
...
describe("writeRubyOverride", () => {
  ...
  it("creates the file on disk", () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "htap-ruby-test-"));
    ...
    expect(content).toContain("htap_ca_path = '/test/ca.pem'");
  });
  ...
  it("returns the file path", () => {
    ...
    expect(filePath).toBe(path.join(tempDir, "htap_intercept.rb"));
