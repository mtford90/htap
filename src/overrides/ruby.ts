 ...
 * OpenSSL::X509::Store to add the httap CA certificate to the default
 * certificate store.
 *
...
export function generateRubyOverrideScript(caCertPath: string): string {
  ...
  return [
    "# Generated by httap — ensures Ruby trusts the proxy CA certificate",
    "",
    "begin",
    "  require 'openssl'",
    "",
    ...
    "",
    "        # Monkey-patch to add httap CA cert alongside system certs",
    "        def set_default_paths",
    "          original_set_default_paths",
    `          httap_ca_path = '${escapedCaCertPath}'`,
    "          if File.exist?(httap_ca_path)",
    "            add_file(httap_ca_path)",
    "          end",
    "        end",
    "      end",
    "    end",
...
export function writeRubyOverride(outputDir: string, caCertPath: string): string {
  ...
  const outputPath = path.join(outputDir, "httap_intercept.rb");
