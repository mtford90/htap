 ...
 * OpenSSL::X509::Store to add the htap CA certificate to the default
 * certificate store.
 *
...
export function generateRubyOverrideScript(caCertPath: string): string {
  ...
  return [
    "# Generated by htap — ensures Ruby trusts the proxy CA certificate",
    "",
    "begin",
    "  require 'openssl'",
    "",
    ...
    "",
    "        # Monkey-patch to add htap CA cert alongside system certs",
    "        def set_default_paths",
    "          original_set_default_paths",
    `          htap_ca_path = '${escapedCaCertPath}'`,
    "          if File.exist?(htap_ca_path)",
    "            add_file(htap_ca_path)",
    "          end",
    "        end",
    "      end",
    "    end",
...
export function writeRubyOverride(outputDir: string, caCertPath: string): string {
  ...
  const outputPath = path.join(outputDir, "htap_intercept.rb");
